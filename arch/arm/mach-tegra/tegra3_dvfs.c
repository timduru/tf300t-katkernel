...
#define GPUMAX 520000
#else
#define GPUMAX 416000
#endif
...
static struct dvfs cpu_dvfs_table[] = {
	...
	CPU_DVFS("cpu_g",  5, 0, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1540, 1540, 1700),
	CPU_DVFS("cpu_g",  5, 1, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1540, 1540, 1700),
	CPU_DVFS("cpu_g",  5, 2, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1540, 1540, 1700),
#endif
...
static struct dvfs core_dvfs_table[] = {
...
#ifdef CONFIG_TF300T_OC
	CORE_DVFS("emc",    2, 1, KHZ,   102000, 450000, 450000, 450000, 450000,  667000,  667000,  677000,  667000),
#else
	...
	CORE_DVFS("vi",     3, 1, KHZ,        1,      1,      1,      1,      1,       1,  470000,  470000,  470000),
        ...
        CORE_DVFS("vde",    0, 1, KHZ,        1, 228000, 275000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("mpe",    0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("2d",     0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("epp",    0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("3d",     0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("3d2",    0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("se",     0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),

        CORE_DVFS("vde",    1, 1, KHZ,        1, 228000, 275000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("mpe",    1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("2d",     1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("epp",    1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("3d",     1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("3d2",    1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("se",     1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
	...
	CORE_DVFS("host1x", 3, 1, KHZ,        1,      1,      1,      1,      1,       1,  242000,  242000,  242000),
        ...
        CORE_DVFS("cbus",   0, 1, KHZ,        1, 228000, 275000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),
        CORE_DVFS("cbus",   1, 1, KHZ,   200000, 228000, 275000, 332000, 380000,  GPUMAX,  GPUMAX,  GPUMAX,  GPUMAX),

#ifdef CONFIG_TF300T_OC
	CORE_DVFS("cbus",   2, 1, KHZ,   200000, 228000, 275000, 332000, 380000,  520000,  520000,  520000,  520000),
#else
