...
#define CPU_MILLIVOLTS {\
        750, 762, 775, 787, 800, 825, 837, 850, 862, 875, 887, 900, 912, 916, 925, 937, 950, 962, 975, 987, 1000, 1012, 1025, 1037, 1050, 1062, 1075, 1087, 1100, 1112, 1125, 1137, 1150, 1162, 1175, 1187, 1200, 1250, 1300, 1350}; 
...
static struct dvfs cpu_dvfs_table[] = {
	...
	CPU_DVFS("cpu_g",  7, 0, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1600, 1700, 1800),
	CPU_DVFS("cpu_g",  7, 1, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1600, 1700, 1800),
	CPU_DVFS("cpu_g",  7, 2, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1600, 1700, 1800),
	CPU_DVFS("cpu_g",  7, 3, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  910,  910,  910,  910,  910,  910,  910, 1150, 1150, 1230, 1230, 1230, 1280, 1280, 1330, 1330, 1370, 1370, 1400, 1400, 1470, 1470, 1500, 1500, 1500, 1500, 1600, 1700, 1800),
	CPU_DVFS("cpu_g",  7, 4, MHZ,   1,   1,   1,   1, 550, 550, 550, 770, 770, 770, 770,  940,  940,  940,  940,  940,  940,  940, 1160, 1160, 1240, 1240, 1240, 1280, 1280, 1360, 1360, 1390, 1390, 1470, 1470, 1500, 1500, 1520, 1520, 1520, 1590, 1600, 1700, 1800),
...
static struct dvfs core_dvfs_table[] = {
	...
	CORE_DVFS("vi",     3, 1, KHZ,        1,      1,      1,      1,      1,       1,  470000,  470000,  470000),
        ...
        CORE_DVFS("vde",    0, 1, KHZ,        1, 228000, 275000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("mpe",    0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("2d",     0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("epp",    0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d",     0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d2",    0, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("se",     0, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),

        CORE_DVFS("vde",    1, 1, KHZ,        1, 228000, 275000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("mpe",    1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("2d",     1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("epp",    1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d",     1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d2",    1, 1, KHZ,        1, 234000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("se",     1, 1, KHZ,        1, 267000, 285000, 332000, 380000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),

        CORE_DVFS("vde",    2, 1, KHZ,        1, 247000, 304000, 352000, 400000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("mpe",    2, 1, KHZ,        1, 247000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("2d",     2, 1, KHZ,        1, 267000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("epp",    2, 1, KHZ,        1, 267000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d",     2, 1, KHZ,        1, 247000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("3d2",    2, 1, KHZ,        1, 247000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
        CORE_DVFS("se",     2, 1, KHZ,        1, 267000, 304000, 361000, 408000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
	...
	CORE_DVFS("host1x", 0, 1, KHZ,   100000, 152000, 188000, 222000, 254000,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2),
	CORE_DVFS("host1x", 1, 1, KHZ,   100000, 152000, 188000, 222000, 254000,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2),
	CORE_DVFS("host1x", 2, 1, KHZ,   100000, 152000, 188000, 222000, 254000,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2,  GPU_CLK_DVFS/2),
	CORE_DVFS("host1x", 3, 1, KHZ,        1,      1,      1,      1,      1,       1,  242000,  242000,  242000),

	CORE_DVFS("cbus",   0, 1, KHZ,   200000, 247000, 304000, 352000, 400000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
	CORE_DVFS("cbus",   1, 1, KHZ,   200000, 247000, 304000, 352000, 400000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
	CORE_DVFS("cbus",   2, 1, KHZ,   200000, 247000, 304000, 352000, 400000,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS,  GPU_CLK_DVFS),
	CORE_DVFS("cbus",   3, 1, KHZ,        1,      1,      1,      1,      1,       1,  484000,  484000,  484000),

	CORE_DVFS("pll_c",  -1, 1, KHZ,  533000, 667000, 667000, 800000, 800000, GPU_CLK_DVFS*2, GPU_CLK_DVFS*2, GPU_CLK_DVFS*2, GPU_CLK_DVFS*2),
