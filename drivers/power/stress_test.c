...
static void battery_strees_test(struct work_struct *work)
{
       ...
       int ret=0;
       struct pad_device_info *battery_device =container_of(work, struct pad_device_info,battery_stress_test.work);

	 ret=pad_smbus_read_data(REG_STATUS,0);
	if (ret < 0) {
	...
	switch(cmd)
	{
	       case BATTERY_POLLING_DATA:
		    if ((arg==START_NORMAL)||(arg==START_HEAVY)){
				 ...
				 queue_delayed_work(battery_work_queue , &pad_device->battery_stress_test, 2*HZ);
			}
		else{
				 printk(" BATTERY:  battery stress test end\n");
				 cancel_delayed_work_sync(&pad_device->battery_stress_test);
			}
		break;
