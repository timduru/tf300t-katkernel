...
static void battery_strees_test(struct work_struct *work)
{
	...
	queue_delayed_work(bq27541_battery_work_queue , &battery_device->battery_stress_test, 2*HZ);
}

long bq27541_battery_ioctl(struct file *filp,unsigned int cmd, unsigned long arg)
{
       int fc=0;
	...
	switch(cmd)
	{
	       case BATTERY_POLLING_DATA:
		    if ((arg==START_NORMAL)||(arg==START_HEAVY)){
				 ...
				 queue_delayed_work(bq27541_battery_work_queue , &bq27541_device->battery_stress_test, 2*HZ);
			}
		else{
...
int bq27541_battery_open(struct inode *inode, struct file *filp)
{
	return 0;
}
struct file_operations bq27541_battery_fops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = bq27541_battery_ioctl,
	.open = bq27541_battery_open,
};
