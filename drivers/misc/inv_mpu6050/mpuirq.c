...
DECLARE_WAIT_QUEUE_HEAD(mpu6050_irq_wait);
...
static int mpu6050_irq_open(struct inode *inode, struct file *file)
{
...
static int mpu6050_irq_release(struct inode *inode, struct file *file)
{
	dev_dbg(mpuirq_dev_data.dev->this_device, "mpu6050_irq_release\n");
	return 0;
}

/* read function called when from /dev/mpuirq is read */
static ssize_t mpu6050_irq_read(struct file *file,
			   char *buf, size_t count, loff_t *ppos)
{
	...
	if (!mpuirq_dev_data.data_ready &&
		...
		wait_event_interruptible_timeout(mpu6050_irq_wait,
...
unsigned int mpu6050_irq_poll(struct file *file, struct poll_table_struct *poll)
{
	int mask = 0;

	poll_wait(file, &mpu6050_irq_wait, poll);
	if (mpuirq_dev_data.data_ready)
...
/* ioctl - I/O control */
static long mpu6050_irq_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int retval = 0;
	int data;
...
static irqreturn_t mpuirq_handler(int irq, void *dev_id)
{
	...
	wake_up_interruptible(&mpu6050_irq_wait);
...
const struct file_operations mpu6050_irq_fops = {
	.owner = THIS_MODULE,
	.read = mpu6050_irq_read,
	.poll = mpu6050_irq_poll,

	.unlocked_ioctl = mpu6050_irq_ioctl,
	.open = mpu6050_irq_open,
	.release = mpu6050_irq_release,
};

static struct miscdevice mpuirq_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = MPUIRQ_NAME,
	.fops = &mpu6050_irq_fops,
};

int mpu6050_irq_init(struct i2c_client *mpu_client, struct mldl_cfg *mldl_cfg)
{
	int res;
