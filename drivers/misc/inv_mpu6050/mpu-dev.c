...
struct mpu6050_private_data *mpu6050_private_data;

static ssize_t read_mpu6050_chip_version(struct device *dev, struct device_attribute *devattr, char *buf)
{	struct i2c_client *client = to_i2c_client(dev);
	struct mpu6050_private_data *data = i2c_get_clientdata(client);
	return sprintf(buf, "%s\n", data->mpu_ver);
}
static ssize_t mpu6050_enable_load_accel_config(struct device *dev, struct device_attribute *devattr, char *buf)
{       struct i2c_client *client = to_i2c_client(dev);
        struct mpu6050_private_data *data = i2c_get_clientdata(client);
...
static ssize_t mpu6050_enable_load_mag_config(struct device *dev, struct device_attribute *devattr, char *buf)
{       struct i2c_client *client = to_i2c_client(dev);
        struct mpu6050_private_data *data = i2c_get_clientdata(client);
        mpu6050_flagLoadConfig = false;
        return sprintf(buf, "%d\n", mpu6050_flagLoadConfig);
}

static ssize_t mpu6050_read_compass_status(struct device *dev, struct device_attribute *devattr, char *buf)
{	struct i2c_client *client = to_i2c_client(dev);
	struct mpu6050_private_data *data = i2c_get_clientdata(client);
	return sprintf(buf, "%d\n", data->compass_status);
}

static ssize_t mpu6050_read_accel_status(struct device *dev, struct device_attribute *devattr, char *buf)
{	struct i2c_client *client = to_i2c_client(dev);
	struct mpu6050_private_data *data = i2c_get_clientdata(client);
	return sprintf(buf, "%d\n", data->accel_status);
}

static ssize_t mpu6050_read_gyro_status(struct device *dev, struct device_attribute *devattr, char *buf)
{	struct i2c_client *client = to_i2c_client(dev);
	struct mpu6050_private_data *data = i2c_get_clientdata(client);
	return sprintf(buf, "%d\n", data->gyro_status);
}

static ssize_t mpu6050_read_accel_raw(struct device *dev, struct device_attribute *devattr, char *buf)
{	struct i2c_client *client = to_i2c_client(dev);
...
{	struct i2c_client *client = to_i2c_client(dev);
	...
	if(mldl_cfg->slave[EXT_SLAVE_TYPE_ACCEL]){
		...
		inv_mpu6050_serial_single_write(client->adapter, mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address,
				0x1B, 0x92);
				inv_mpu6050_serial_single_write(client->adapter, mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address,
				0x1D, 0x40);

		res = inv_mpu6050_serial_read(client->adapter, mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address,
					0x10, 6, data);
		if(res){
...
DEVICE_ATTR(6050_accel_status, S_IRUGO, mpu6050_read_accel_status, NULL);
DEVICE_ATTR(6050_gyro_status, S_IRUGO, mpu6050_read_gyro_status, NULL);
DEVICE_ATTR(6050_enLoadMagConfig, S_IRUGO, mpu6050_enable_load_mag_config, NULL);
DEVICE_ATTR(6050_enLoadAccelConfig, S_IRUGO, mpu6050_enable_load_accel_config, NULL);
DEVICE_ATTR(mpu6050_version, S_IRUGO, read_mpu6050_chip_version, NULL);

static struct attribute *mpu_6050_attr[] = {
	&dev_attr_6050_gyro_status.attr,
	&dev_attr_6050_accel_status.attr,
...
{
	struct mpu6050_private_data *mpu = (struct mpu6050_private_data *)data;
...
{
	struct mpu6050_private_data *mpu =
	    container_of(nb, struct mpu6050_private_data, nb);
	struct i2c_client *client = mpu->client;
...
{
	struct mpu6050_private_data *mpu =
	    container_of(file->private_data, struct mpu6050_private_data, dev);
	struct i2c_client *client = mpu->client;
	int result;
	int ii;
	dev_dbg(&client->adapter->dev, "%s\n", __func__);
...
static int mpu_release(struct inode *inode, struct file *file)
{
	...
	result = inv_mpu6050_suspend(mldl_cfg,
...
{
	struct mpu6050_private_data *mpu =
	    (struct mpu6050_private_data *)i2c_get_clientdata(client);
			...
			struct ext_slave_config __user *usr_config)
{
	...
	retval = mpu6050_gyro_config(gyro_adapter, mldl_cfg, &config);
	kfree(config.data);
	return retval;
}
			    ...
			    struct ext_slave_config __user *usr_config)
{
	...
	retval = mpu6050_gyro_get_config(gyro_adapter, mldl_cfg, &config);
	if (!retval)
			...
			struct ext_slave_config __user *usr_config)
{
	...
	retval = inv_mpu6050_slave_config(mldl_cfg, gyro_adapter, slave_adapter,
				      &config, slave, pdata);
	kfree(config.data);
	return retval;
}
			    ...
			    struct ext_slave_config __user *usr_config)
{
	...
	retval = inv_mpu6050_get_slave_config(mldl_cfg, gyro_adapter,
			  ...
			  void __user *usr_data)
{
	...
	retval = inv_mpu6050_slave_read(mldl_cfg, gyro_adapter, slave_adapter,
				    slave, pdata, data);
			   ...
			   struct mpu_read_write __user *usr_msg)
{
	...
	switch (cmd) {
	case MPU_READ:
		retval = inv_mpu6050_serial_read(sl_handle, addr,
					 msg.address, msg.length, msg.data);
		break;
	case MPU_WRITE:
		retval = inv_mpu6050_serial_write(sl_handle, addr,
					  msg.length, msg.data);
		break;
	case MPU_READ_MEM:
		retval = inv_mpu6050_serial_read_mem(sl_handle, addr,
					     msg.address, msg.length, msg.data);
		break;
	case MPU_WRITE_MEM:
		retval = inv_mpu6050_serial_write_mem(sl_handle, addr,
					      msg.address, msg.length,
					      msg.data);
		break;
	case MPU_READ_FIFO:
		retval = inv_mpu6050_serial_read_fifo(sl_handle, addr,
					      msg.length, msg.data);
		break;
	case MPU_WRITE_FIFO:
		retval = inv_mpu6050_serial_write_fifo(sl_handle, addr,
					       msg.length, msg.data);
		break;
...
void mpu6050_shutdown(struct i2c_client *client)
{
	struct mpu6050_private_data *mpu =
	    (struct mpu6050_private_data *)i2c_get_clientdata(client);
	struct mldl_cfg *mldl_cfg = &mpu->mldl_cfg;
	...
	mutex_lock(&mpu->mutex);
	(void)inv_mpu6050_suspend(mldl_cfg,
			slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE],
			slave_adapter[EXT_SLAVE_TYPE_ACCEL],
			slave_adapter[EXT_SLAVE_TYPE_COMPASS],
			slave_adapter[EXT_SLAVE_TYPE_PRESSURE],
...
int mpu6050_dev_suspend(struct device *dev, pm_message_t mesg)
{
	printk("%s+\n", __FUNCTION__);
...
int mpu6050_dev_resume(struct device *dev)
{
	printk("%s+\n", __FUNCTION__);
	struct i2c_client *client = i2c_verify_client(dev);
	struct mpu6050_private_data *mpu =
	...
	if (mpu->pid && !mldl_cfg->inv_mpu_cfg->ignore_system_suspend) {
		(void)inv_mpu6050_resume(mldl_cfg,
				slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE],
				slave_adapter[EXT_SLAVE_TYPE_ACCEL],
				slave_adapter[EXT_SLAVE_TYPE_COMPASS],
				slave_adapter[EXT_SLAVE_TYPE_PRESSURE],
...
int inv_mpu6050_register_slave(struct module *slave_module,
			struct i2c_client *slave_client,
			struct ext_slave_platform_data *slave_pdata,
			struct ext_slave_descr *(*get_slave_descr)(void))
{
	struct mpu6050_private_data *mpu = mpu6050_private_data;
	struct mldl_cfg *mldl_cfg;
...
out_unlock_mutex:
	...
	if (!result && irq_name && (slave_pdata->irq > 0)) {
		...
		dev_info(&slave_client->adapter->dev,
			...
			irq_name,
			slave_pdata->irq);
		warn_result = mpu6050_slaveirq_init(slave_client->adapter,
...
EXPORT_SYMBOL(inv_mpu6050_register_slave);

void inv_mpu6050_unregister_slave(struct i2c_client *slave_client,
			struct ext_slave_platform_data *slave_pdata,
			struct ext_slave_descr *(*get_slave_descr)(void))
{
	struct mpu6050_private_data *mpu = mpu6050_private_data;
	...
	if (slave_pdata->irq)
		mpu6050_slaveirq_exit(slave_pdata);
...
EXPORT_SYMBOL(inv_mpu6050_unregister_slave);
...
int mpu6050_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	printk(KERN_INFO "%s+ #####\n", __func__);
	...
	mpu = kzalloc(sizeof(struct mpu6050_private_data), GFP_KERNEL);
	if (!mpu) {
		res = -ENOMEM;
	...
	res = inv_mpu6050_open(&mpu->mldl_cfg, client->adapter, NULL, NULL, NULL);
	...
	if (client->irq) {
		...
		res = mpu6050_irq_init(client, mldl_cfg);
		if (res)
	...
	if (!strcmp(mpu_id[0].name, devid->name)) {
		/* Special case to re-use the inv_mpu6050_register_slave */
		struct ext_slave_platform_data *slave_pdata;
		...
		res = inv_mpu6050_register_slave(
			NULL, client,
			slave_pdata,
			mpu6050_get_slave_descr);
		if (res) {
			/* if inv_mpu6050_register_slave fails there are no pointer
...
static const struct dev_pm_ops mpu_dev_pm_ops={
	.suspend = mpu6050_dev_suspend,
	.resume = mpu6050_dev_resume,
};
...
static struct i2c_driver mpu6050_driver = {
	...
	.shutdown = mpu6050_shutdown,	/* optional */
};

static int __init mpu_init(void)
{
	printk(KERN_INFO "%s(6050)+ #####\n", __func__);
	int res = i2c_add_driver(&mpu6050_driver);
